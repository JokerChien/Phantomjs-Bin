...
var url='http://www.abchina.com/cn/PersonalServices/SvcBulletin/';
	...
	if (status !== 'success') {  
		...
		window.setTimeout(function () {
			...
			var outPut=page.evaluate(
				function(){
					return document.getElementsByClassName("details_rightWrapC");
				}
			)
			...
			for(i=0;i<outPut.length;i++){
				str_txt+=outPut[i].innerHTML.replace(/href=\".+?\"/g,"").replace(/>\s*/g,">").replace(/\s*</g,"<");
			...
			// console.log(page.content);
			FileSave('<h2>农业银行</h2>'+str_txt+'<hr>');
			phantom.exit();
		}, 3333);
    }
	// phantom.exit();
...
function FileSave(theContent){
	...
	var path		='C:\\Bank_Notieces\\ABC_Notice.txt';
	var path_diff	='C:\\Bank_Notieces\\ABC_Notice_diff.txt';
	...
	if(fs.exists(path)){	//如果存在则将文件内容进行比较。
		...
		if(theContent==content_LastTime){
			fs_diff.write(path_diff,'农业银行：今日无更新','w');
		}else{
			fs_diff.write(path_diff,'农业银行：今日有更新注意查看','w');
		}
	}
